...
int main(int argc, char *argv[]) {
    ...
    for(int i = 0; i<ruta.getPuntos().size();i++) {
        ...
        for(Paises::Iterator it = paises.begin(); it != paises.end() && !encontrado; it++){
            if(i%2 == 0 &&ruta[i].getX() == (*it).getPunto().getX() && ruta[i].getY() == (*it).getPunto().getY()) {
                double punto_medio_x = (*it).Bandera().Filas()/2;
                double punto_medio_y = (*it).Bandera().Columnas()/2;
                mapa.PutImagen((*it).getPunto().getX()-punto_medio_x,(*it).getPunto().getY()-punto_medio_y, (*it).Bandera(), BLENDING);
                encontrado = true;
            }
        }
        //Descomentar esta parte para obtener aviones de llegada a cada pais
        /*if(i%2 == 0 && i != 0 && i != ruta.getPuntos().size()-1) {
        ...
        double rot_radianes = atan2(ruta[pos_segundo].getX() - ruta[pos_primero].getX(), ruta[pos_segundo].getY() - ruta[pos_primero].getY());
        //Rotar aviones y pegarlos
        Imagen avion_rotado = Rota(avion, rot_radianes);
        mapa.PutImagen(ruta[i].getX()-avion_rotado.Filas()/2, ruta[i].getY()-avion_rotado.Columnas()/2, avion_rotado, OPACO);
    }
